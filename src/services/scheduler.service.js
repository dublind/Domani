...
class SchedulerService {
  ...
  async exportYesterdaySales() {
    try {
      ...
      try {
        const mmResult = await marketmanService.uploadDailySales(products, {
          totalSinImpuesto,
          totalConImpuesto,
          ordenes: result.data.length
        }, dateStr);
        ...
        if (mmResult.success) {
          logger.info('Ventas enviadas a Marketman correctamente');
        } else {
          logger.warn(`Error enviando a Marketman: ${mmResult.error}`);
        }
      } catch (err) {
        logger.error('Excepción al subir a Marketman:', err);
      }
  ...
  async runRangeExport(startDate, endDate) {
    ...
    // También intentar subir este rango a Marketman (si está configurado)
    try {
      const mmResult = await marketmanService.uploadDailySales(products, {
        totalSinImpuesto,
        totalConImpuesto,
      ...
      if (mmResult.success) {
        logger.info('Ventas de rango enviadas a Marketman correctamente');
      } else {
        logger.warn(`Error enviando ventas de rango a Marketman: ${mmResult.error}`);
      }
    } catch (err) {
      logger.error('Excepción al subir rango a Marketman:', err);
    }
  ...
  async runManualExport(date = null) {
    if (date) {
      ...
      // También intentar subir a Marketman (si está configurado)
      try {
        const mmResult = await marketmanService.uploadDailySales(products, {
          totalSinImpuesto,
          totalConImpuesto,
        ...
        if (mmResult.success) {
          logger.info('Ventas enviadas a Marketman correctamente (manual)');
        } else {
          logger.warn(`Error enviado a Marketman (manual): ${mmResult.error}`);
        }
      } catch (err) {
        logger.error('Excepción al subir manualmente a Marketman:', err);
      }
