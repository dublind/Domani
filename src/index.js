  ...
  res.send(`
    ...
    <body>
      <div class="container">
        ...
        <div class="card">
          <div class="form-row">
            ...
            <button class="btn btn-test" onclick="sendToMarketman()" id="btnMM">Enviar a Marketman</button>
          </div>
        </div>
      ...
      <script>
        ...
        async function sendToMarketman() {
          if (!lastStart || !lastEnd) return;
          const resultDiv = document.getElementById('result');
          resultDiv.style.display = 'block';
          resultDiv.innerHTML = '<p class="loading">Enviando datos a Marketman...</p>';
          try {
            const response = await fetch('/api/marketman/trigger?date=' + lastStart);
            const data = await response.json();
            if (data.success) {
              resultDiv.innerHTML = '<div class="card" style="border-color:rgba(74,222,128,0.3);"><p style="color:#4ade80;font-weight:bold;text-align:center;font-size:18px;">Envío a Marketman completado</p></div>';
            } else {
              resultDiv.innerHTML = '<div class="error">Error: ' + (data.error || 'No se pudo enviar') + '</div>';
            }
          } catch (error) {
            resultDiv.innerHTML = '<div class="error">Error: ' + error.message + '</div>';
          }
...
// Endpoint para pruebas de Marketman (revisa configuración, no hace llamada real)
app.get('/api/marketman/test', (req, res) => {
  const configured = marketmanService.isConfigured();
  res.json({ success: configured, configured });
});

// Endpoint para disparar manualmente la exportación y subida a Marketman
app.get('/api/marketman/trigger', async (req, res) => {
  try {
    const { date } = req.query;
    logger.info(`Trigger Marketman manual${date ? ` para fecha: ${date}` : ''}`);
    const result = await schedulerService.runManualExport(date || null);
    res.json(result);
  } catch (error) {
    logger.error('Error en trigger Marketman:', error);
